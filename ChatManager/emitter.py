...
def expand_env(s: Any) -> Any:
    """Expands ${VARS} inside strings using os.environ (missing vars -> "")."""
...
def main() -> None:
    ...
    # Allow the SSN session to be sourced from /bot/.env via ${SSN_SESSION}.
    # Also allow direct env override if you prefer setting SSN_SESSION in the shell.
    ssn_session = str(ssn_cfg.get("session", "") or "").strip()
    ssn_session = expand_env(ssn_session)
    ssn_session = (os.getenv("SSN_SESSION") or "").strip() or ssn_session
    ...
    # Optional direct env overrides
    overlay_chat_file_raw = (os.getenv("OVERLAY_CHAT_FILE") or "").strip() or overlay_chat_file_raw
    overlay_events_file_raw = (os.getenv("OVERLAY_EVENTS_FILE") or "").strip() or overlay_events_file_raw
