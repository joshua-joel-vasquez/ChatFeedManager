...
async def run_writer():
    ...
    # This keeps all sensitive config in one place.
    bot_root = Path(__file__).resolve().parents[1]
    shared_env = bot_root / ".env"
    if shared_env.exists():
        load_dotenv(shared_env)
    else:
        # Fallback (lets you run from SSNChatWriter/ if you want)
        load_dotenv()

    session = (os.getenv("SSN_SESSION") or "").strip()
    if not session:
        raise RuntimeError("SSN_SESSION missing. Add it to /bot/.env (see /bot/.env.example).")
